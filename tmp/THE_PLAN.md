...
| G2 | 🔧 IN PROGRESS |  | TPROC-C build (1 warehouse) → run (1 VU, 2 min rampup, 5 min duration) → non-zero NOPM/TPM, on CI | Build path is wired up via the new "Phase G" workflow steps: download HammerDB-5.0-Prod-Win.tar.gz, overlay our `src/firebird/`, `config/firebird.xml`, `config/database.xml`, `scripts/{tcl,python}/firebird/`, then run `hammerdbcli auto fb_hammerdbcli_ci.tcl` with `fb_count_ware=1`, `fb_num_vu=1`. Run path (vurun + NOPM/TPM) still pending. |

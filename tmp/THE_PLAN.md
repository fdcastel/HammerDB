...
| C5 | 🔧 IN PROGRESS |  | TPROC-C PSQL stored procedures: `NEWORD`, `PAYMENT`, `DELIVERY`, `OSTAT`, `SLEV` | Initial cut: `PAYMENT_SP` implemented in `fb_tpcc_sp_ddl` (fboltp.tcl). Demonstrates the install path (`CREATE OR ALTER PROCEDURE` via `tdbc::odbc`, no `SET TERM` needed) and EXECUTE PROCEDURE invocation. CI step `Install + smoke PSQL stored procedures` installs and invokes against the loaded warehouse. NEWORD/DELIVERY/OSTAT/SLEV pending — they need PSQL loops + cursors and are larger ports. |

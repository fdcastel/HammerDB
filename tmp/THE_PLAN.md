...
| C3 | 🔧 IN PROGRESS |  | TPROC-C schema DDL: `CREATE TABLE` for WAREHOUSE, DISTRICT, CUSTOMER, ITEM, STOCK, ORDERS, ORDER_LINE, NEW_ORDER, HISTORY | DDL implemented in `fb_tpcc_table_ddl` + `fb_tpcc_index_ddl` (fboltp.tcl). 9 tables + 2 secondary indexes match the canonical TPC-C set used by other HammerDB drivers. CI step `Build TPC-C schema on a fresh embedded .fdb` builds + verifies via `RDB$RELATIONS`/`RDB$INDICES` queries. No identity columns yet — TPC-C uses caller-supplied keys (D_NEXT_O_ID etc.). |

...
| C5 | 🔧 IN PROGRESS | 2b1a91b | TPROC-C PSQL stored procedures: `NEWORD`, `PAYMENT`, `DELIVERY`, `OSTAT`, `SLEV` | **PAYMENT_SP fully verified end-to-end.** PSQL body in `.github/workflows/scripts/fb_payment_sp.sql`; install via `Invoke-FirebirdIsql` with `SET TERM ^` (tdbc::odbc cannot submit a CREATE PROCEDURE body that uses `:NAME` PSQL variable references); invocation via `SELECT FROM PAYMENT_SP(:params)` (selectable proc, `SUSPEND`-based). CI run output: `OUT_C_BALANCE -22.34 OUT_C_CREDIT BC OUT_W_NAME ... OUT_D_NAME ...` for customer 1 / -12.34 payment. NEWORD/DELIVERY/OSTAT/SLEV are larger ports (loops, cursors, per-line work) — deferred pending bandwidth. |

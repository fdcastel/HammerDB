...
| C8 | 🔧 IN PROGRESS |  | Update-conflict retry logic for MVCC | Helper `fb_with_retry` + classifier `fb_is_retryable_error` in fboltp.tcl. Pattern-matches `deadlock` / `lock conflict` / `update conflict` / `concurrent update` / `concurrent transaction` / `40001` / `isc_update_conflict` (case-insensitive). All 5 client-side driver procs (neword/payment/delivery/ostat/slev) wrapped in `for {attempt 0..3} ... continue` loop with 5-40ms randomised back-off; delivery does per-district retry so one conflict doesn't abort the others. CI step `MVCC retry helper unit tests` exercises `fb_is_retryable_error` against retryable + non-retryable samples and verifies `fb_with_retry` retries-then-succeeds, propagates non-retryables, and gives up after max_retries. |

...
| C11 | 🔧 IN PROGRESS |  | TPROC-H driver: bulk loader + power test | Loader done (see prior). Power test added: `fb_tpch_rf1` (insert SF*1500 new orders + 1-7 lineitems each), `fb_tpch_rf2` (delete by orderkey, round-trips RF1), `fb_tpch_sub_query` (port of postgres `sub_query` — fills :N placeholders per spec, with date/string/numeric branches per query), `fb_tpch_power_test` (RF1 → 22 queries in `ordered_set $myposition` order → RF2; returns per-query timings, rows, gmean). CI step `Run full TPROC-H power test` reuses the loaded .fdb and asserts: RF1/RF2 round-trip, all 22 queries execute, ≥1 returns rows, gmean > 0. Multi-VU "throughput" test still pending. |

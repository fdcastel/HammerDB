...
| C11 | ✅ DONE | 91220c2 | TPROC-H driver: bulk loader + query stream | All 8 loaders in fbolap.tcl (`fb_tpch_load_*`); orchestrator `fb_load_tpch`. Uses `tpchcommon` helpers + ISO date format from `mk_time_bcp`. CI step `Load TPC-H at scale 0.01 + run a few queries` loads ~87k rows in 12s and runs Q1/Q5/Q14 against the data. CI verified: REGION=5, NATION=25, SUPPLIER=100, CUSTOMER=1500, PART=2000, PARTSUPP=8000, ORDERS=15000, LINEITEM=60018; Q1 returned 4 rows in 238ms, Q5 0 rows (no matches at this scale), Q14 1 row. CI script overrides `tpchcommon::PART_SUPP_BRIDGE` with an integer-coerced version (the upstream uses `%` which fails on fractional scales). Refresh streams (RF1/RF2) and multi-VU stream coordination still pending — separate follow-up. |

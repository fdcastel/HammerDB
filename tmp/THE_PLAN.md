...
| C10 | 🔧 IN PROGRESS |  | TPROC-H queries 1–22 in Firebird SQL dialect | All 22 queries ported in `fb_tpch_queries` (fbolap.tcl): PG `interval ':1 day'` → Firebird `dateadd(-:1 day to date '...')`, `interval '3 month'` / `'1 year'` → `dateadd(... to date)`, `LIMIT N` not used (queries don't have it). Q15 multi-statement (view + select + drop view) preserved with `:VID` substitution slot. CI step `PREPARE all 22 TPC-H queries` PREPAREs each against the empty schema as a syntax/column check. |

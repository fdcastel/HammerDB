...
| C11 | 🔧 IN PROGRESS |  | TPROC-H driver: bulk loader + query stream | Bulk loader (`fb_tpch_load_*` for region/nation/supplier/customer/part_partsupp/orders_lineitem) implemented in fbolap.tcl using prepared INSERTs + named params + 1000-row batches; reuses `tpchcommon` helpers (`mk_time_bcp`, `mk_sparse`, `pick_str_1`, `gen_phone`, `V_STR`, `TEXT_1`, `PART_SUPP_BRIDGE`, `rpb_routine`). `fb_load_tpch $conn $scale` is the orchestrator. CI step `Load TPC-H at scale 0.01 + run a few queries` builds the schema, loads ~80k rows, asserts per-table row counts, then PREPAREs+executes Q1/Q5/Q14 against the data. Refresh streams (RF1/RF2) and multi-VU stream coordination still pending. |

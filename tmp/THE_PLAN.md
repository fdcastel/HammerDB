...
| C4 | 🔧 IN PROGRESS |  | TPROC-C schema bulk-load procs (no COPY/BCP available) | Procs added to fboltp.tcl: `fb_load_item`, `fb_load_warehouse`, `fb_load_districts`, `fb_load_customer_history`, `fb_load_stock`, `fb_load_orders`, `fb_load_warehouse_data`. Prepared INSERT + named tdbc params; 1000-row batches with intermediate commits. CI step `Load 1 warehouse worth of TPC-C data` builds + loads + asserts row counts. |
